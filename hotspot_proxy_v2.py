...
class Proxy(DatagramProtocol):
    ...
    def reaper(self,_peer_id):
        ...
        if self.clientinfo and _peer_id != b'\xff\xff\xff\xff':
            print(f"{datetime.now().replace(microsecond=0)} Client: ID:{str(int_id(_peer_id)).rjust(9)} IP:{_peer['shost'].rjust(15)} Port:{_peer['sport']} Removed.")
