        ...
        if _command == DMRD:    # DMRData -- encapsulated DMR data frame
            ...
            if _data[:5] == RPTCL:    # Disconnect command
                ...
                if _peer_id in self._peers \
                    ...
                    self.send_peer(_peer_id, b''.join([RPTACK, _peer_id]))
                    logger.info('(%s) Peer %s (%s) has sent repeater configuration, Package ID: %s, Software ID: %s', self._system, _this_peer['CALLSIGN'], _this_peer['RADIO_ID'],self._peers[_peer_id]['PACKAGE_ID'].decode(),self._peers[_peer_id]['SOFTWARE_ID'].decode())
                else:
                    self.transport.write(b''.join([MSTNAK, _peer_id]), _sockaddr)
