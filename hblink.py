    ...
    def datagramReceived(self, _packet, _sockaddr):
        ...
        if _packet[:4] == DMRD:    # DMRData -- encapsulated DMR data frame
            ...
            if compare_digest(_hash, _ckhs) and _sockaddr == self._config['TARGET_SOCK']:
                ...
                h,p = _sockaddr
                logger.info('(%s) OpenBridge HMAC failed, packet discarded - OPCODE: %s DATA: %s HMAC LENGTH: %s HMAC: %s %s %s', self._system, _packet[:4], repr(_packet[:53]), len(_packet[53:]), repr(_packet[53:]),h,p) 
