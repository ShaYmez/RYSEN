    ...
    def dmrd_received(self, _peer_id, _rf_src, _dst_id, _seq, _slot, _call_type, _frame_type, _dtype_vseq, _stream_id, _data):
        ...
        if _call_type == 'group' or _call_type == 'vcsbk':
            ...
            if self.STATUS[_slot]['packets'] > 1 and _pkt_crc in self.STATUS[_slot]['crcs']:
                logger.warning("(%s) *PacketControl* DMR packet payload with Crc32: %s seen before in this stream, disgarding. Stream ID:, %s TGID: %s, SEQ: %s, packets: ",self._system,_pkt_crc,int_id(_stream_id),int_id(_dst_id),_seq,self.STATUS[_slot]['packets'])
                return
            #Inbound out-of-order packets
