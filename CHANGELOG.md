...
- `POST /device/drop-dynamic` and `/device/disconnect` clear only that hotspot's RF/dial links and mute that radio's RX of those TGs. The shared MASTER leg stays up if another radio still has the talkgroup. Disconnect additionally queues one-shot `DISC=1`; neither action logs the HBP session out. `POST /device/drop-call` mutes this radio for the current over (`stream_id`) whether it is talking or only listening; the next over after VTERM is forwarded again.
